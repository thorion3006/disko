...
''; in {
  options = {
    ...
    content = diskoLib.deviceType { parent = config; device = "${lib.concatStringsSep ":" (lib.traceVal toplevel-config.disko.devices._internal.xbpools).${config.name}}"; inherit toplevel-config; };
    _meta = lib.mkOption {
      internal = true;
      readOnly = true;
      type = diskoLib.jsonType;
      default =
        lib.optionalAttrs (config.content != null) (config.content._meta [ "xbpool" config.name ]);
        ...
        ${lib.optionalString (config.passwordFile == null && config.askPassword) ''
          askPassword() {
            if [ -z "''${IN_DISKO_TEST+x}" ]; then
              set +x
              echo "Enter the password for the disk encryption: "
          ...
          ${config.mountpoint} = ''
            ...
            ${lib.optionalString (config.passwordFile == null && config.askPassword) ''
              askPassword() {
                if [ -z "''${IN_DISKO_TEST+x}" ]; then
                  set +x
                  echo "Enter the password to unlock the disk: "
                  IFS= read -r -s password
