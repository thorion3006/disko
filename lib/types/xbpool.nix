  ...
  options = {
    ...
    _create = diskoLib.mkCreateOption {
      ...
      default = ''
        ...
        ${lib.optionalString (config.passwordFile != null) "bcachefs unlock ${config.name} <<<\"$(cat ${config.passwordFile})\""}
        ${lib.concatMapStrings (subvol: ''
          (
            MNTPOINT=$(mktemp -d)
            mount ${config.name} "$MNTPOINT"
    ...
    _mount = diskoLib.mkMountOption {
      ...
      default = {
        fs = lib.optionalAttrs (config.mountpoint != null) {
          ${config.mountpoint} = ''
            ${lib.optionalString (config.passwordFile != null) "bcachefs unlock ${config.name} <<<\"$(cat ${config.passwordFile})\""}
            mount -t bcachefs ${config.name} "${rootMountPoint}${config.mountpoint}" \
