...
''; in {
  options = {
    ...
    content = diskoLib.deviceType { parent = config; device = "${lib.concatStringsSep ":" (lib.traceVal toplevel-config.disko.devices._internal.bpools).${config.name}}"; inherit toplevel-config; };
    _meta = lib.mkOption {
      internal = true;
      readOnly = true;
      type = diskoLib.jsonType;
      default =
        lib.optionalAttrs (config.content != null) (config.content._meta [ "bpool" config.name ]);
