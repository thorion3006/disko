{ config, options, diskoLib, lib, parent, device, ... }:
{
  options = {
    type = lib.mkOption {
    ...
    _meta = lib.mkOption {
      ...
      default = dev: {
        deviceDependencies.xbpool.${config.pool} = [ dev ];
      };
    ...
    _create = diskoLib.mkCreateOption {
      ...
      default = ''
        if [ ! -d "$disko_devices_dir"/bcachefs_${config.pool}" ]; then
          mkdir $disko_devices_dir"/bcachefs_${config.pool}
        fi
    ...
    _config = lib.mkOption {
      ...
      default = [
        {
          disko.devices._internal.xbpools.${config.pool} = [ (lib.traceVal config.device) ];
        }
      ];
