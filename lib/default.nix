...
let
  ...
  diskoLib = {
    ...
    */
    ...
    /* topLevel type of the disko config, takes attrsets of disks, mdadms, bpools, zpools, nodevs, and lvm vgs.
    */
    toplevel = lib.types.submodule (cfg:
      let
        devices = { inherit (cfg.config) disk mdadm bpool zpool lvm_vg nodev; };
      in
      {
        options = {
          disk = lib.mkOption {
          ...
          bpool = lib.mkOption {
            type = lib.types.attrsOf diskoLib.types.bpool;
            default = { };
